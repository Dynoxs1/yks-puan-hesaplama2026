...
<style>
    ...
    /* Grafik alanı varsayılan olarak gizli */
    #grafikAlani {
        display: none; 
        margin-top: 20px;
        padding: 20px;
...
<div class="hesaplama-kutusu">
    <h3>YKS Net Hesaplayıcı</h3>
    <div class="form-group">
        <label>Hedef Puanınız (0-500):</label>
        <input type="number" id="floatingHedefPuan" class="form-control" placeholder="Örn: 350">
    </div>
    <div class="form-group">
        <input type="checkbox" id="sadeceTYT">
        <label for="sadeceTYT">Sadece TYT (2 Yıllık / Ön Lisans)</label>
    </div>
    <button onclick="hesaplaFloatingNet()">HESAPLA</button>
    <div class="sonuc-kutusu">
        <div><strong>Gereken TYT Net:</strong> <span id="floatingTytNet">0</span></div>
        <div id="aytSonucSatiri"><strong>Gereken AYT Net:</strong> <span id="floatingAytNet">0</span></div>
    </div>
</div>

<div id="grafikAlani">
    <h4>Grafikler ve Detaylı Analiz</h4>
    <p>Burada grafikleriniz (Chart.js vb.) görünecek.</p>
    </div>

<script>
function hesaplaFloatingNet() {
    // --- VERİLERİ AL ---
    let hedefInput = document.getElementById('floatingHedefPuan');
    let hedef = parseFloat(hedefInput.value) || 0; // Değer yoksa 0 kabul et
    let sadeceTYT = document.getElementById('sadeceTYT').checked;
    
    // HTML Elementleri
    let tytSpan = document.getElementById('floatingTytNet');
    let aytSpan = document.getElementById('floatingAytNet');
    let aytSatiri = document.getElementById('aytSonucSatiri');
    let grafikAlani = document.getElementById('grafikAlani');

    // --- KONTROL 1: HEDEF PUAN SINIRLARI ---
    if (hedef < 0) hedef = 0;
    if (hedef > 500) hedef = 500;
    hedefInput.value = hedef; // Düzeltilen değeri kutuya geri yaz

    // --- KONTROL 2: DEĞER GİRİLMEMİŞSE GRAFİĞİ AÇMA ---
    if (hedef === 0) {
        grafikAlani.style.display = 'none'; // Grafiği gizle
        tytSpan.innerText = "0";
        aytSpan.innerText = "0";
        return; // Fonksiyonu burada bitir, hesaplama yapma
    }

    // Değer varsa grafiği aç
    grafikAlani.style.display = 'block';

    // --- HESAPLAMA MANTIĞI ---
    
    // Not: ÖSYM her öğrenciye taban 100 puan verir. 
    // Gerçekçi hesap için (Hedef - 100) üzerinden net hesabı yapılır.
    let netPuan = (hedef - 100);
    if (netPuan < 0) netPuan = 0;

    if (sadeceTYT) {
        // SENARYO: SADECE TYT
        aytSatiri.style.display = 'none'; // AYT satırını gizle
        
        // Mantık: Puanın tamamını TYT'den almalı.
        // Ortalama 1 TYT Neti ~ 3.3 Puan (Yıla göre değişir)
        let tytGereken = netPuan / 3.3; 
        
        tytSpan.innerText = Math.round(tytGereken);
        aytSpan.innerText = "0"; // Arka planda 0 kalsın

    } else {
        // SENARYO: TYT + AYT (Lisans)
        aytSatiri.style.display = 'block'; // AYT satırını göster
        
        // Mantık: Puanın %40'ı TYT, %60'ı AYT'den gelir.
        // Ortalama AYT Neti ~ 3.0 Puan (Sayısal/EA karışık kaba hesap)
        
        let tytKatkisi = netPuan * 0.40;
        let aytKatkisi = netPuan * 0.60;

        // Katsayılar (Yaklaşık)
        let tytGereken = tytKatkisi / 1.4; // TYT katsayısı (yaklaşık)
        let aytGereken = aytKatkisi / 3.0; // AYT katsayısı (yaklaşık)

        tytSpan.innerText = Math.round(tytGereken);
        aytSpan.innerText = Math.round(aytGereken);
    }
}
</script>
...
<div class="w-full bg-white mt-6">
  <img src="images/banner.jpg"
       ...
       class="w-full h-[28vh] sm:h-[86vh] object-contain">
</div>
